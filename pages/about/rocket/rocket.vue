<template>
	<view class="rocket">
		<!-- <image class="image" mode='widthFix' :src="list[this.id].picurl"></image> -->
待完善...
		<!-- 基本信息 -->
		...
		data() {
			return {
				list: [{
						ename: "Falcon 9",
						name: "猎鹰 9 火箭",
						picurl: "/static/image/Falcon 9.png",
					},
					{
						ename: "Falcon Heavy",
						name: "重型猎鹰火箭",
						picurl: "/static/image/Falcon Heavy.png"
					},
					{
						ename: "Starship",
						name: "星际飞船",
						picurl: "/static/image/Starship.png"
					},
					{
						ename: "Dragon",
						name: "龙飞船",
						picurl: "/static/image/Dragon.png"
					}
				]
			};
		},
		onLoad(option) {
			this.Rockets = getApp().globalData.Rockets[option.id];
			this.id = option.id
